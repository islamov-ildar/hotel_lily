  ...
  <div class="card relative shadow">
    <div v-if="data.user_name !=='link_to_yandex'" class="absolute top-[-60px] left-[55px] w-[132px] h-[132px] rounded-t-full bg-[#FFFFFF] flex items-center justify-center">
      <div v-if="data.user_photo !== 'default'" class="w-[90px] h-[90px] rounded-full overflow-hidden">
        <img :src="data.user_photo" alt="person">
      </div>
    ...
    <div v-if="data.user_name !=='link_to_yandex'" class="w-full text-left pt-[30px] pb-[30px] px-[30px] text-blueMain">
      ...
      <div class="font-montserratMedium text-[18px] md:text-[24px]">{{ data.user_name }}</div>
      <div class="font-montserratRegular text-[13px] md:text-[17px] flex gap-[16px] mt-[16px] mb-[25px] items-center">
        <div class="flex">
      ...
      <div :id="`feedback_text-${number}`" class="feedbackText font-montserratRegular text-[13px] md:text-[16px] leading-[22px]">
        {{ data.feedback_text }}
      </div>
      <div @click="$emit('openFeedbackModal', data)" v-if="rowCount === 88" class="readMore mt-[8px] cursor-pointer font-montserratMedium text-[13px] md:text-[16px] flex items-center gap-[7px]">
    ...
    <a v-else href="https://yandex.ru/maps/org/liliya/180430998084/reviews/?ll=40.626525%2C43.097830&z=17" target="_blank" class="readMore h-full flex items-center justify-between mx-[55px] cursor-pointer">
      <div class="flex">
        <div class="font-montserratMedium text-[13px] md:text-[16px] text-blueMain pr-[8px]">
